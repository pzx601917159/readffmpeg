...
// 不支持选项
static const AVOption pzx_options[] = 
{
    { NULL }
};
...
static const AVClass pzx_class = {
    .class_name = "pzx",
    .item_name = av_default_item_name,
    .option      = pzx_options,
    .version    = LIBAVUTIL_VERSION_INT,
...
static int pzx_check(URLContext *h, int mask)
{
    int ret = 0;
    const char *filename = h->filename;
    av_strstart(filename, "pzx:", &filename);
...
static int pzx_delete(URLContext *h)
{
#if HAVE_UNISTD_H
    int ret;
    const char *filename = h->filename;
...
static int pzx_move(URLContext *h_src, URLContext *h_dst)
{
    const char *filename_src = h_src->filename;
    const char *filename_dst = h_dst->filename;
    av_strstart(filename_src, "pzx:", &filename_src);
    av_strstart(filename_dst, "pzx:", &filename_dst);
...
static int pzx_open(URLContext *h, const char *filename, int flags)
{
    FileContext *c = h->priv_data;
    int access;
    int fd;
...
const URLProtocol ff_pzx_protocol = {
    .name                = "pzx",
    .url_open            = pzx_open,
    .url_read            = file_read,
    .url_write           = file_write,
